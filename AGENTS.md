...
- The composer's agent-settings picker renders whatever the agent advertises, minus the effort option, which the reasoning picker already owns — two controls for one setting would let them disagree with the reasoning picker silently winning every turn. Set options by `id`, not `category`: Claude Code's persona option carries no category at all.
