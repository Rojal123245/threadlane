...
- Keep long chat transcripts on GPUI's variable-height `ListState` and load their canonical JSONL backward in bounded turn-aligned pages. Use `ListState::splice` when prepending older rows so its logical scroll anchor advances with the insertion; do not rebuild every message element or reopen and reduce the complete session for history pagination.
