...
fn native_sources_are_preserved_with_deterministic_precedence() {
    ...
    write_skill(
        &home.path().join(".threadlane/skills/global-threadlane"),
        "global-threadlane",
        "global threadlane",
        "global threadlane body",
    );
    write_native_package(
        &home.path().join(".threadlane/packages/global-package"),
        "global-package",
        "global package body",
    );
    write_skill(
    ...
    write_skill(
        &project.path().join(".threadlane/skills/project-threadlane"),
        "project-threadlane",
        "project threadlane",
        "project threadlane body",
    );
    write_native_package(
        &project.path().join(".threadlane/packages/project-package"),
        "project-package",
    ...
    for root in [
        ...
        home.path().join(".threadlane/skills/duplicate"),
        project.path().join(".agents/skills/duplicate"),
        project.path().join(".threadlane/skills/duplicate"),
    ] {
        write_skill(&root, "duplicate", "duplicate", &root.to_string_lossy());
    }
    write_native_package(
        &home.path().join(".threadlane/packages/duplicate-global"),
        "duplicate",
    ...
    assert_eq!(scopes["global-threadlane"], SkillScope::GlobalThreadlane);
    assert_eq!(scopes["global-package"], SkillScope::GlobalPackage);
    assert_eq!(scopes["project-agents"], SkillScope::ProjectAgents);
    assert_eq!(scopes["project-threadlane"], SkillScope::ProjectThreadlane);
...
async fn yaml_catalog_and_load_tool_are_lazy_and_strict() {
    let project = tempdir().unwrap();
    let skill_directory = project.path().join(".threadlane/skills/yaml-skill");
...
fn rejects_symlinked_native_packages_root_outside_project() {
    ...
    fs::create_dir_all(project.path().join(".threadlane")).unwrap();
    symlink(outside.path(), project.path().join(".threadlane/packages")).unwrap();
