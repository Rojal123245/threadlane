...
pub(crate) fn generation_event_drain_error(error: broadcast::error::TryRecvError) -> Option<&'static str> {
    match error {
...
pub(crate) fn recover_v2_subagent_records(session_file: &Path) -> Result<Vec<HarnessRecord>, String> {
...
/// Adapts the harness journal to the [`AgentJournal`] trait for the main
/// agent lane. Replaces the 8 individual recorder callbacks.
struct HarnessJournalAdapter {
    session_file: PathBuf,
    active_run: Arc<std::sync::Mutex<Option<String>>>,
}

/// Lane-aware journal adapter for subagent child processes.
/// Writes to a specific lane/run_id within the parent's session file.
struct SubagentJournalAdapter {
    session_file: PathBuf,
    lane: String,
    run_id: String,
}
...
impl HarnessJournal {
    ...
    pub(crate) fn record_assistant_attempt(&mut self, run_id: &str, usage: TokenUsage) -> Result<(), String> {
        self.refresh()?;
        let start_seq = self
            .store
            .records()
...
impl CodingAgent {
    ...
    #[cfg(test)]
    pub(crate) fn set_subagent_work_observer(&self, observer: Arc<std::sync::Mutex<Vec<AgentWork>>>) {
    ...
    pub(crate) async fn recover_harness_tool_batch(&mut self, run_id: &str) -> Result<bool, String> {
