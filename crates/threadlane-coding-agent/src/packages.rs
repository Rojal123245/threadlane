...
impl PackageManager {
    ...
    fn scan_packages_dir(&self, dir: &Path, scope: PackageScope, out: &mut Vec<PackageRecord>) {
        ...
        for entry in entries.flatten() {
            let path = entry.path();
            if path.is_dir() {
                let manifest_file = path.join("threadlane-package.json");
    ...
    fn install_from_local(
        ...
        let manifest_file = source_path.join("threadlane-package.json");
        if !manifest_file.exists() {
            return Err("Source directory does not contain 'threadlane-package.json'".into());
        }
        ...
        let target_dir = match scope {
            ...
            PackageScope::Project => {
                ...
                proj.join(".threadlane/packages").join(&manifest.id)
            }
        };
    ...
    fn remove_package(
        ...
        let target_dir = match scope {
            ...
            PackageScope::Project => {
                ...
                proj.join(".threadlane/packages").join(package_id)
            }
        };
