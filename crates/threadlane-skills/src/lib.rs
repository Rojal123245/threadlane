...
impl Discovery {
    ...
    fn run(&mut self) {
        ...
        if let Some(home) = home_dir.as_deref() {
            ...
            self.scan_skill_directory(
                &home.join(".threadlane/skills"),
                SkillScope::GlobalThreadlane,
                Some(home),
            );
            self.scan_native_packages(
                &home.join(".threadlane/packages"),
        ...
        if let Some(project) = project_root.as_deref() {
            ...
            self.scan_skill_directory(
                &project.join(".threadlane/skills"),
                SkillScope::ProjectThreadlane,
                Some(project),
            );
            self.scan_native_packages(
                &project.join(".threadlane/packages"),
    ...
    fn scan_native_packages(
        ...
        for package_path in package_entries.into_iter().filter(|path| path.is_dir()) {
            ...
            let manifest_path = package_root.join("threadlane-package.json");
    ...
    fn process_skill_file(&mut self, file_path: &Path, allowed_root: &Path, scope: SkillScope) {
        ...
        let (frontmatter, parse_error) = match parsed {
            ...
            Err(error) => (ParsedFrontmatterMeta::default(), Some(error)),
        };
...
struct FrontmatterDocument {
    metadata: ParsedFrontmatterMeta,
    body_offset: usize,
}
...
fn parse_frontmatter_document(
    ...
    let mut metadata: ParsedFrontmatterMeta =
