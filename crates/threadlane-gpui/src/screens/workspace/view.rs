...
impl WorkspaceView {
    ...
    fn render_environment_popover(&self, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let total_additions: u32 = status.map(|s| s.files.iter().map(|f| f.additions).sum()).unwrap_or(0);
        let total_deletions: u32 = status.map(|s| s.files.iter().map(|f| f.deletions).sum()).unwrap_or(0);
        let has_changes = total_additions > 0 || total_deletions > 0 || status.map(|s| s.has_changes).unwrap_or(false);
        let branch_name = status.and_then(|s| s.branch.clone()).unwrap_or_else(|| "main".to_string());
